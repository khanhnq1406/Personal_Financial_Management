...
function generateAPIClient(services) {
  ...
  for (const service of services) {
    ...
    for (const method of service.methods) {
      ...
      if (httpMethod === "get") {
        // Special handling for specific GET endpoints
        if (method.name === "VerifyAuth") {
          ...
          // Build query params for list requests
          ...
          // Build query params for other GET requests
          apiCode.push(`      const params = new URLSearchParams();`);
        ...
        // Map request fields for REST
        if (method.name === "Login" || method.name === "Register") {
          // Map googleToken -> token
...
// Main function
function main() {
  console.log("🚀 Generating REST API client from proto files...");
  ...
  console.log(`✨ Generated API client: ${outputFile}`);
  console.log("");
